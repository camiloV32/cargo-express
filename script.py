...
def getToken(repartidor):
  url = "http://localhost:8000/api/obtener_token_delivery/"
  headers = {
    "Content-Type": "application/json"
  }
  response = requests.post(url, json=repartidor, headers=headers)
  json_response = response.json()
  return json_response['token']
def registrar_pedido_entregado(pedido_id, repartidor, productos):
    url = "http://localhost:8000/api/registrar_pedido_entregado/"
    token = getToken(repartidor)
    payload = {
        "pedido_id": pedido_id,
...
while True:
    ...
    productos_choice = random.choices(productos, k=random.randint(1, 10))

    registrar_pedido_entregado(pedido_id, repartidor, productos_choice)
    time.sleep(5)